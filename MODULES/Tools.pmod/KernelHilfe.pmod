...
string typeof_token(string|array token)
{
  ...
  array tokenize(string input, function|void debug_callback)
  {
      array tokens = Parser.Pike.split(input);
      if (debug_callback)
          debug_callback(sprintf("\n\ntokenize(%O): %O\n\n", input, tokens));
...
class Evaluator {
  ...
  array get_completions(string input, function|void debug_callback) {
    ...
    mixed error = catch
    {
      tokens = tokenize(input, debug_callback);
    };
    ...
    {
      array completable = get_resolvable(tokens, debug_callback);

      if (completable && sizeof(completable))
      {
        error = catch
        {
          completions = get_module_completions(completable, debug_callback);
        };
        error = Error.mkerror(error);
      }
      ...
      if (debug_callback)
        debug_callback(sprintf("input: %s\ntokens: %O\ncompletable: %O\ncompletions: %O\n", input, tokens, completable, completions));
    }

   if(stringp(completions)) completions = ({completions});
    handler->show_errors();
    handler->show_warnings();
  ...
  array|string get_file_completions(string path, function|void debug_callback)
  {
    array files = ({});
    if ( (< "", ".", ".." >)[path-"../"] )
      files += ({ ".." });
  ...
  array|string get_module_completions(array completable, function|void debug_callback)
  {
    ...
    if (debug_callback)
      debug_callback(sprintf("get_module_completions(%O): %O, %O, %O\n", completable, base, rest, type));
    array completions = low_get_module_completions(rest, base, type, space, debug_callback);
  ...
  array low_get_module_completions(array completable, object base, void|string type, void|int(0..1) space, function|void debug_callback)
  {
      if (debug_callback)
        debug_callback(sprintf("low_get_module_completions(%O\n, %O, %O, %O)\n", completable, base, type, space));
      ...
      else if (!base)
      {
          ...
          {
            if (debug_callback)
              debug_callback("autodoc without base\n");
            return ({});
          }
...
class StdinHilfe
{
  ...
  void handle_completions(string key)
  {
    ...
    mixed err = catch(completions = get_completions(input));
